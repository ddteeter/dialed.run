...
| D-33 | **Does a garment carry a type?** The pack draws per-*type* glyphs (singlet, tee, longSleeve, halfZip, jacket, vest, shorts, halfTights, tights, cap, beanie, headband…) and design's P2 tap-list is a list of the same types. `garmentSchema` is a discriminated union on eight broad *categories*, so `{category:"top", layer:"base"}` cannot choose between singlet, tee and longSleeve — sleeve length is not modelled. Same for shorts/halfTights/tights (leg length) and cap/beanie (sun vs warmth). Not a design question: P2 and the pack already agree, and asking for eight category-level glyphs would put the same icon on all sixteen tap-list rows. Contract-shaped, so it stops with the owner (CLAUDE.md schema protocol). Resolves `sportsBra`/`armSleeves`, which are provisional glyphs and real P2 rows but not categories. **Does not block the tap-list**: `TAP_LISTS` entries already encode their type, and want a `glyph` field rather than a derivation — `mild-long-sleeve` is a quarter-zip, so the key alone lies. | `lib/contracts.ts`, `db/schema-core.ts`, `modules/closet/tap-list.ts` | blocked | Before lane 105 builds onboarding, or before any closet surface shows a garment glyph. |
