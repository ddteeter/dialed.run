...
| D-38 | ~~Mutation testing cannot work in the Workers pool.~~ **Solved — it drills through in ten lines.** Stryker's instrumented code activates a mutant by reading `globalThis.process.env.__STRYKER_ACTIVE_MUTANT__`, which the command runner sets on the child process; inside the workers pool that object is the Worker's **bindings**, not the parent environment, so no mutant ever activated and everything survived. But `vitest.config.ts` runs in Node, where the variable *is* visible — forwarding it as a miniflare binding carries it across. Same file, same tests: **0.00 with 10/10 survived → 90.00 with 9 killed**, and the one survivor was a real gap (a symbol replaced by `" "` vs `""`, only observable between words), now tested and at 100%. Run it with `npm run mutate`. Cost, measured: ~11s per mutant with the whole suite as the command (10 mutants in 109s at concurrency 4), so a full `src/lib` sweep is over an hour — narrow `mutate` to the file you are working on rather than narrowing the command, because a command that misses a mutated file reports 0.00 and that reads as a finding. Still **not** in the commit gate, and `@stryker-mutator/vitest-runner` still fails — it spawns its own vitest, which lacks the pool's module rules and dies on a binary fixture. | `vitest.config.ts`, `stryker.conf.json` | done | — |
| D-39 | **`.dependency-cruiser.cjs` does not exclude `.stryker-tmp/`, and now that `npm run mutate` exists this bites during a run.** Stryker's sandbox is a full copy of the repo, so the commit gate reports `only-env-touches-cloudflare` against a *copied* `src/env/index.ts` — a real architecture rule, naming a file nobody wrote. `premutate`/`postmutate` clear the directory around a run, which covers the interrupted-run case, but a commit made *while* a run is in flight still fails. The config also states it does not read `.gitignore`, where `.stryker-tmp/` already is. It is a forbidden zone, so it needs the owner: add `\\.stryker-tmp/` to `exclude`. Filed upstream as agentic-guardrails-scaffolding#55. | `.dependency-cruiser.cjs` | blocked | Owner decision — one line. |
