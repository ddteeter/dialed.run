...
| D-25 | **Adopt `lib/copy.ts` in the auth surfaces** — folded into D-17. `RETRY_GENERIC` is inlined in `routes/auth/login.tsx`, `signup.tsx` and `modules/auth/google-button.tsx`. Doing it alone was tried and reverted: touching those two routes makes the dupes gate report their form markup, which is a real 22-line clone that the Forms & failure primitives delete outright. Fixing it twice is worse than fixing it once. | `modules/auth/`, `routes/auth/` | ready | With D-17, in the same edit. |
...
| D-40 | **87 surviving mutants in `src/lib`** — the baseline the tooling now makes visible. 62.03% over 316 mutants: `contracts.ts` 53% (52 survivors) and `temperature.ts` 14% (9 survivors, 21 uncovered) are most of it; `normalize.ts`, `owned.ts` and `photo-constraints.ts` are already 100%. Each survivor is a line whose behaviour no test pins — the first one looked at was real (`replaceAll(/\p{Symbol}/gu, " ")` vs `""`, which decides whether "Nike®Air" and "Nike Air" collapse onto one canonical brand row). Triaging this is also the precondition for putting stryker in the commit gate, which is otherwise blocked by its own backlog. | `src/lib/**` | ready | Before turning `stryker` on in `guardrails.config.json`. |
