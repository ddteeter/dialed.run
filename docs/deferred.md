...
| D-38 | **Mutation testing cannot work in the Workers pool, and the mechanism is now known.** `stryker: "off"` was an adoption default set when there were almost no tests. With 275 the precondition changed, so it was measured — and the result was worse than "slow": the generic `command` runner reports **every mutant as survived** (0.00 on `lib/normalize.ts`, where the same mutation applied by hand fails four assertions). Root cause, traced: stryker instruments source with a switch that reads `globalThis.process.env.__STRYKER_ACTIVE_MUTANT__`, and the command runner sets that variable on the child process. `@cloudflare/vitest-pool-workers` runs tests inside workerd isolates whose `process.env` is built from the Worker's **bindings**, not inherited from the parent — verified: with the variable exported, `typeof process` is `object`, `process.env` has 10 keys, and the stryker variable is absent. So no mutant ever activates. `@stryker-mutator/vitest-runner` fails earlier still, on the dry run. A green-looking 0.00 is the dangerous part: it reads as "no coverage" when it means "no mutation". | `guardrails.config.json`, `vitest.config.ts` | blocked | A second vitest project running pure `lib/` modules on the node pool — that is where mutation testing would pay anyway, since the Workers-bound code is mostly I/O. |
| D-39 | **`.dependency-cruiser.cjs` does not exclude `.stryker-tmp/`.** Guardrails seeds both that config and stryker's, and stryker's sandbox is a full copy of the repo — so a sandbox left on disk fails the commit gate with a violation inside a *copied* `src/env/index.ts`. It cost a confusing gate failure while measuring D-38. The config also states outright that it does not read `.gitignore`, where `.stryker-tmp/` already is. Filed upstream as agentic-guardrails-scaffolding#55; the local fix is one line in a human-managed file. | `.dependency-cruiser.cjs` | blocked | Before `stryker` is ever set to anything but `off`. |
