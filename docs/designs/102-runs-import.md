...
idempotency, failed parse, duplicate, DLQ⇒failed+notified); `webhook.test`
(challenge echo, valid event⇒notification, unknown athlete 200 no-op,
invalid payload 200+logged); `strava.test` (unit: token refresh ok/broken);
`notifications.test` (dedupe UNIQUE, unread count, mark-read).
