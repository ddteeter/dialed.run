...
/**
 * The LLM rung, over the chat-completions wire format (packet §5).
 *
 ...
 * Its answer is parsed through the same contract as every other rung, and
 * it lands only where the declared rungs left a blank.
 *
 * **One adapter, two endpoints.** The packet asked for OpenRouter so that
 * one key could cover every model the eval compares, and the eval still
 * goes through it. Production does not (PR #72 review): the eval landed on
 * one model, `gpt-5.6-luna`, and once the choice is a single OpenAI model
 * an intermediary is a second party that can be down, a routing layer whose
 * only job here was to be told not to route, and a margin on every call.
 * So the consumer talks to OpenAI directly, with its own key. The request
 * shape is the same on both — OpenRouter is OpenAI-compatible by design —
 * which is why this is one file with an endpoint parameter rather than two
 * adapters, and why the eval's provider pin is the one field that differs.
 *
 * **Why the pin exists at all.** Structured output is per *endpoint*, not
 * per model (confirmed at design time): OpenRouter serves `gpt-5.6-luna`
 * from seven endpoints and Amazon Bedrock's ignores `response_format`,
 * answering with prose. That is not an error; it is a successful completion
 * nobody can parse. Through OpenRouter the provider is pinned and fallbacks
 * are off. Against OpenAI directly there is one endpoint, and the field is
 * not sent — OpenAI rejects request arguments it does not know.
 */
...
export interface ChatCompletionsOptions {
  /**
  Where the request goes: `OPENAI_ENDPOINT` in production, `OPENROUTER_ENDPOINT` in the eval.
  */
  endpoint: string;
  /**
  The model id, as the endpoint names it — `gpt-5.6-luna` to OpenAI, `openai/gpt-5.6-luna` to OpenRouter.
  */
  model: string;
  /**
   * The provider to pin, with fallbacks off. An OpenRouter field: set it
   * there, because structured output is per endpoint and a fallback can
   * route to one without it. Leave it unset against OpenAI, which refuses
   * request arguments it does not know.
   */
...
async function askModel(
  ...
  const response = await fetchImpl(options.endpoint, {
    ...
    body: JSON.stringify({
      model: options.model,
      // Pinned, and fallbacks off, where there is a router to pin — see the
      // note above on per-endpoint structured output.
  ...
  if (!response.ok) {
    ...
    // is the provider's complaint, and the first one said "'propertyNames'
    // is not permitted" — which is the difference between a fix and a week
    // of guessing. It carries no secret; the key is a request header, never
    // echoed.
